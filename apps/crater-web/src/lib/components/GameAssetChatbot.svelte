...
<script lang="ts">
  ...
  function updateTitleWithMessage(
    sessionIdToUpdate: string,
    text: string
  ): void {
    updateSession(sessionIdToUpdate, (session) => {
  ...
  function applyProviderForMode(mode: SessionMode): void {
    if (mode === 'chat') {
      chatbotService.setAIProvider(
        chatProvider && chatProvider.isConfigured() ? chatProvider : undefined
      )
      return
    }

    chatbotService.setAIProvider(
      imageProvider && imageProvider.isConfigured() ? imageProvider : undefined
    )
  }
  ...
  async function saveImageToS3(
    ...
    try {
      ...
      // Upload to S3
      await s3Service.uploadImageFromUrl(imageUrl, filename, {
        prompt,
  ...
  function updateAIProviders(): void {
    ...
    if (
      aiProvider !== 'debug' &&
      (!apiKey || !isValidApiKey(aiProvider, apiKey))
    ) {
      imageProvider = null
      chatProvider = null
  ...
  function loadSettings(): void {
    ...
    try {
      ...
      aiProvider = ['openai', 'gemini', 'debug'].includes(storedProvider)
        ? (storedProvider as ProviderKind)
        : 'debug'
      ...
      chatModel = settings.chatModel ?? getDefaultModel(aiProvider, 'chat')
...
<div class="chatbot-container">
  ...
  <div class="chat-body">
    <ChatHeader {activeSession} {providerStatus} onClearSession={clearChat}>
